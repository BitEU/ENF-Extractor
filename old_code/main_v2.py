...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].lower() == "help":
        ...
        Optional arguments:
            ...
            -w, --window          Analysis window length in seconds (default: 2.0).
            ...
            -r, --overlap         Window overlap ratio (0-1, default: 0.5).
    ...
    parser.add_argument('-f', '--frequency', type=float, default=60.0,
                        help='Nominal mains frequency (default: 60 Hz). Set to 50 for most of Europe/Asia, 60 for Americas. Does not affect processing time or accuracy directly.')
    parser.add_argument('-t', '--tolerance', type=float, default=0.5,
                        help='Frequency search tolerance (+/- Hz, default: 0.5 Hz). Higher values increase processing time and may reduce accuracy by including more noise, but can help if the ENF deviates significantly from nominal.')
    parser.add_argument('-w', '--window', type=float, default=2.0,
                        help='Analysis window length in seconds (default: 2.0). Increasing window length improves frequency resolution and confidence, but increases processing time per window and reduces the number of output points (lower time resolution).')
    parser.add_argument('-r', '--overlap', type=float, default=0.5,
                        help='Window overlap ratio (0-1, default: 0.5). Higher overlap increases the number of output points and smoothness, but increases total processing time.')
    parser.add_argument('--sample-rate', type=int, default=22050,
                        help='Audio sample rate (default: 22050). Higher sample rates may improve accuracy for high-frequency harmonics, but increase memory usage and processing time.')
    parser.add_argument('--harmonics', nargs='+', type=int, default=[1, 2, 3],
                        help='Harmonics to analyze (default: 1 2 3). Including more harmonics can improve robustness and confidence, but increases processing time.')
